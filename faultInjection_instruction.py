...
#    cuda_gdb_p.expect(CUDA_GDB_EXPECT)
  ...
  #-------------------------------------------------------------------------------------------------------------------------------------
    ...
    while i == 0:
        ...
        if int(pc_c) != pc+off_set:
            #cuda_gdb_p.sendline(STEPI)
            #k = cuda_gdb_p.expect([pexpect.TIMEOUT,CUDA_GDB_EXPECT],timeout=120)
            #if k == 0:
            logger.info("Cannot hit pc,modified by sudeep here to check single-stepping")
            killProcess(configure.benchmark)
            time.sleep(2)
            cuda_gdb_p.terminate(force=True)
            #cuda_gdb_p.close()
...
def main():
    ...
    for trial in range(1000):
        ...
        faultMain(configure.binary_path,trial,pc,str(profile.keys()[kernel]),iteration,kernel,item) # last two are for breakpoint function
        ft_end = datetime.now()
        logger.info("This run takes "+str(ft_end-ft_start))
        args="rm "+configure.trialfile
        runDiff(args)
        time.sleep(5)
